...
from .resnet18_layers import conv, linear
...
class ResNet(nn.Module):
    def __init__(
        ...
        self.conv1 = conv(
            3, num_features[0], 7, 2, 3,
            activation, use_batchnorm=use_batchnorm,
            bias=False)
        if self.use_maxpool:
        ...
        blocks = []
        blocks.append(
            self.build_blocks(block, num_features[0], num_features[0], 1, num_blocks[0]))
        for i in range(1, len(num_blocks)):
            blocks.append(
                self.build_blocks(
                    block, num_features[i - 1], num_features[i], 2, num_blocks[i]))
        ...
        if feature_layer_dim is not None:
            self.fc1 = linear(
                num_features[-1] * shape, feature_layer_dim, use_batchnorm=use_batchnorm
            )
