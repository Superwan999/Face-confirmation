...
def weight_initialization(weight, activation):
    ...
    elif isinstance(activation, nn.LeakyReLU) or isinstance(activation, nn.PReLU):
...
class BasicBlock(nn.Module):
    def __init__(
        ...
        self.conv1 = conv(
            in_channels, out_channels, 3, stride, 1, activation, use_batchnorm
        )
        self.conv2 = conv(out_channels, out_channels, 3, 1, 1, None, use_batchnorm)
        self.downsample = None
        if stride != 1 or in_channels != out_channels:
            self.downsample = conv(
                in_channels, out_channels, 1, stride, 0, None, use_batchnorm
            )
